...
<div class="pure-g" style="text-align:left;">
    ...
    <div class="pure-u-1-3" style="margin: 0px auto; background:#996633; width:98%; padding: 1% 1% 10% 1%; height:auto; background-image:url('<?php echo URL::site('assets',true,false); ?>/shoppingcartIcon.png'); background-repeat:no-repeat; background-position:bottom right;">
    	<!-- Panel Form -->
    	<div class='pure-g'>
    	...
    	<div id='productsListahan' class='pajinateDiv' style='margin: 5px;'>
    		...
    		<ul class='content'>
    			...
    			<div id='productScore' class='productDetails' style='float:right; text-align: right;'><h1><?php echo $p['product_name']; ?></h1><p class='productDesc'><?php echo $p['description']; ?></p><br>
    			<b>PRICE: </b><i>PHP <?php echo $p['price']; ?></i><br>
    			<!-- GETS Product's overall score (Nested Loop per Loop of products) -->
    			<?php $queryGetOverall = DB::query(DATABASE::SELECT, "SELECT ROUND(AVG(score)) AS overall FROM crm_product_scores WHERE product_no = '".$p['product_no']."'")->execute(); ?>
    			<?php foreach($queryGetOverall AS $overall){ ?>
    			<div class='rateit' data-productno='<?php echo $p['product_no']; ?>' data-rateit-value='<?php echo $overall['overall']; ?>'></div>
    			<b class='boldRate'><?php echo (isset($overall['overall'])) ? $overall['overall'] : 0; ?>/5</b>
    			<?php } ?>
    			<!-- END -->
    			</div></div></li>
				<script language='javascript'>
				// Client Side Script for submition of product rate
				$("#productScore .rateit").bind('rated reset', function(e){
					var ri = $(this);
...
<script language='javascript'>
	$(document).ready(function(){
		...
		$('#closeProductSurvey').on('click',function(){
			$.ajax({
				url: '<?php echo URL::base((isset($_SERVER['HTTPS'])) ? 'https' : 'http', true); ?>products/close_modal',
				type: 'POST',
		...
		$('.buttonAddProduct').on('click', function(){
			var unqid = makeid();
			$('.productsInquiry').append('<div class="' + unqid + '"><select name="products_inquiry[]" style="border: 1px #ffffff;">' + options + '</select> <input class="buttonAddProduct" type="button" name="inqCancel" value="Cancel" onclick="$(\'.' + unqid + '\').remove();"/><br></div>');
		});
		// AJAX Validation of Inquiry Form
		$(".validationInquiryResponse").hide();
// 		$(".productsInquiryValidation").hide();

		// validation of cilent type
// 		var client_type = null;
		...
		// Submits HTTP Request on realtime with AJAX here below
		// Past Client's products inquiry via AJAX
		...
		$("input[name='submitInqForm']").on('click', function(){
			...
			$.ajax({
				url: '<?php echo URL::base((isset($_SERVER['HTTPS'])) ? 'https' : 'http', true); ?>crm/validate_inquiry',
				type: 'POST',
				data: {
