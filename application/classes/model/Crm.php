...
class Model_Crm extends Model_Database {
	...
	public function get_inquiry_informations($datas = array(), $inquiry_code) {
		...
		DB::query ( DATABASE::INSERT, "INSERT INTO crm_inq_docs VALUES('" . $inquiry_code . "',now(),null, '0')" )->execute ();
	...
	public function insert_feedback($data = array()) {
		DB::query ( DATABASE::INSERT, "INSERT INTO crm_feedback VALUES(SHA1(NOW()),'" . $data ['users_ip'] . "','" . $data ['recommendation'] . "','" . $data ['no_statement'] . "','" . $data ['comment'] . "','" . $data ['satisfaction'] . "','" . $data ['business_rate'] . "','" . $data ['order_product'] . "')" )->execute ();
	}
	...
	public function select_product_scores() {
		return DB::query ( DATABASE::SELECT, "SELECT crm_product_scores.*,crm_products.* FROM crm_product_scores JOIN crm_products ON crm_products.`product_no` = crm_product_scores.`product_no` ORDER BY crm_product_scores.`score_code` ASC" )->execute ()->as_array ();
	}
	...
	public function deny_selected_inquiry($inquiry_no = null) {
		DB::query ( DATABASE::UPDATE, "UPDATE crm_inq_docs SET inq_status = 0 WHERE inq_no = '" . $inquiry_no . "'" )->execute ();
	}
	...
	// Login bussiness processing layer
	public function login($data = array()) {
	}
	// Admin Side of CRM
	public function add_sales_assistant($data = array()) {
		DB::query ( DATABASe::INSERT, "INSERT INTO crm_accounts VALUES(null,'" . $data [3]/*strtolower ( $data [0] . "." . $data [2] )*/ . "',sha1('" . $data [2] . "'),'" . $data [0] . "','" . $data [1] . "','" . $data [2] . "','2')" )->execute ();
	}
