...
class Controller_Products extends Controller {
	...
	public function action_index() {
		...
		$homepage->survey_form = (empty ( $this->session->get ( 'survey_ask' ) )) ? $this->obj ['forms']->product_survey_confirmation () : null;
	...
	public function action_product_survey($survey = 0) {
		...
		if(!$this->request->query('survey_code')){
			echo "<script>self.location='".URL::site('',true,false)."products';</script>";
		}
	...
	public function action_submit_survey() {
		...
		echo "<script>self.location = '".URL::site('products', null, false)."?survey_answered=".hash('sha256','success')."';</script>";
	}
