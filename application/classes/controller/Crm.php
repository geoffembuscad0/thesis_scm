...
class Controller_Crm extends Controller {
	...
	public function action_export_survey() {
		...
		<script type="text/javascript" src="' . URL::site ( "assets/js/jqplot/jquery.jqplot.js" ) . '"></script>
	...
	public function action_admin_add_sales() {
		if (Valid::alpha ( $this->request->post ( 'firstname' ) )) {
			if (Valid::alpha ( $this->request->post ( 'middlename' ) ) || empty ( $this->request->post ( 'middlename' ) )) {
	...
	public function action_validate_inquiry() {
		...
		$data_entries = array (
				'client_type' => (empty ( $this->request->post ( 'client_type' ) )) ? "<b>Type of client:</b> Please select what type of client are you. " : $this->request->post ( 'client_type' ),
				...
				'client_address' => $this->validate_address ( explode ( ",", $this->request->post ( 'address' ) ) ),
